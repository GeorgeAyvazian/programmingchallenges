...
public class ThreeNPlusOne
{
        ...
        {
                final List<Pair<Integer, Integer>> pairs = readInput();
                final Map<Pair<Integer, Integer>, Integer> map = new HashMap<Pair<Integer,Integer>, Integer>(){
                        @Override
                ...
                {
                        final Integer limit = pair.getValue();
                        final List<Integer> ints = new ArrayList<>();
                        for (int i = pair.getKey(); i <= limit; i++)
                        {
                                int j = i;
                                if (j == 1)
                                {
                        ...
                        map.put(pair, ints.get(ints.size() - 1));
                }
        ...
        private static List<Pair<Integer, Integer>> readInput()
        {
                try (FileInputStream fileInputStream = new FileInputStream("input.txt"))
                {
                        ...
                        final String[] pairs = stringBuilder.toString().split("\n");
                        final Pattern pattern = Pattern.compile("(\\d+)\\s*(\\d+)");
                        final List<Pair<Integer, Integer>> intPairs = new ArrayList<>();
                        for (String pair : pairs)
                        {
                                final Matcher matcher = pattern.matcher(pair);
                                matcher.find();
                                intPairs.add(new Pair<Integer, Integer>(Integer.valueOf(matcher.group(1)), Integer.valueOf(matcher.group(2))) {
                                        @Override
                                        public String toString()
                                        {
